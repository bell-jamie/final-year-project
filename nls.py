    ...
    def __call__(
        ...
        * Setting `conf.is_linear == True` means a pre-assembled and possibly
        ...
        while 1:
            ...
            while 1:
                ...
                if ok:
                    ...
                    red = conf.ls_red
                    output(
                        "linesearch: iter %d, (%.5e < %.5e) (new ls: %e)"
                    ...
                    output(
                        "residual computation failed for iter %d"
                        " (new ls: %e)!" % (it, red * ls)
                    )
            ...
            if not conf.is_linear:
                ...
                mtx_a, tau = ensure_positive_definite(mtx_a, tau)
            else:
        ...
        if conf.report_status:
            output(
                f"cond: {condition}, iter: {it}, ls_iter: {ls_n_iter},"
...
class ScipyBroyden(NonlinearSolver):
    ...
    _parameters = [
        ...
        ("alpha", "float", 0.9, False, "See ``scipy.optimize``."),
        ("M", "float", 5, False, "See ``scipy.optimize``."),
        ("f_tol", "float", 1e-6, False, "See ``scipy.optimize``."),
        ("w0", "float", 0.1, False, "See ``scipy.optimize``."),
    ]
    ...
    def __call__(
        ...
        kwargs = {"iter": conf.i_max, "alpha": conf.alpha, "verbose": conf.verbose}
...
class PETScNonlinearSolver(NonlinearSolver):
    ...
    _parameters = [
        ("method", "str", "newtonls", False, "The SNES type."),
        ("i_max", "int", 10, False, "The maximum number of iterations."),
        ("if_max", "int", 100, False, "The maximum number of function evaluations."),
        (
            "eps_a",
            "float",
            1e-10,
    ...
    def __call__(
        ...
        ksp.setTolerances(
            atol=ls_conf.eps_a,
            rtol=ls_conf.eps_r,
            divtol=ls_conf.eps_d,
            max_it=ls_conf.i_max,
        )
        ...
        snes.setTolerances(
            atol=conf.eps_a, rtol=conf.eps_r, stol=conf.eps_s, max_it=conf.i_max
        )
        ...
        output(
            "%s(%s): %d iterations in the last step"
            % (ksp.getType(), ksp.getPC().getType(), ksp.getIterationNumber()),
            verbose=conf.verbose,
        )

        output(
            "%s convergence: %s (%s, %d iterations, %d function evaluations)"
            % (
                snes.getType(),
                snes.reason,
                reason,
                snes.getIterationNumber(),
                snes.getFunctionEvaluations(),
            ),
            verbose=conf.verbose,
        )
        ...
        if conf.report_status:
            output(
                f"cond: {condition}, iter: {n_iter}, ls_iter: {ls_n_iter},"
                f" err0: {err0:.8e}, err: {err:.8e}"
            )
            output("solver: %.8f [s]" % status["time_stats"]["solver"])
